...
export const RiskGauge: React.FC<RiskGaugeProps> = ({ score, level, confidence, size = 'md' }) => {
  const getScoreColor = (s: number) => {
    ...
    if (s >= 70) return 'text-orange-400 stroke-orange-400';
    if (s >= 35) return 'text-yellow-400 stroke-yellow-400';
    return 'text-emerald-400 stroke-emerald-400';
  };

  const getScoreBg = (s: number) => {
    if (s >= 90) return 'bg-red-500/10 border-red-500/30';
    if (s >= 70) return 'bg-orange-500/10 border-orange-500/30';
    if (s >= 35) return 'bg-yellow-500/10 border-yellow-500/30';
